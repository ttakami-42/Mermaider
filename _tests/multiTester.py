...
class multiTestClass():
	def __init__(self, model: str, promptKeyValue: str, needTool: int, fileName: str, count: int):
		...
		self.model = model
		self.inputJson = utils.loadJsonFile(f'data/input/{fileName}')
		self.promptKeyValue = promptKeyValue
		self.needTool = needTool

		# enlighten setting
		self.manager = enlighten.get_manager()
		self.pbar = self.manager.counter(total=len(self.inputJson.keys()) * count, desc='Ticks', unit='ticks')

		# class for openpyxl
		self.data = utils.DataClass(keyValue=promptKeyValue, fileName=os.path.splitext(fileName)[0], title1='filename', title2='input')

	def multiTester(self, inputkeyValue: str, text: str, row: int) -> None:
		try:
			...
			if response['isSuccess']:
				...
				status, url = -1, 'N/A'
			self.data.writeCell(row=row, status=status, completion=response['completion'], prompt=response['prompt'], total=response['total'], content=response['content'], url=url, inputKeyValue=inputkeyValue, text=text)
...
if __name__ == '__main__':
	...
	if len(args) == 5 and args[2].isdigit() and args[4].isdigit() and int(args[2]) in [0, 1]:
		...
		if "GPT-4" in promptKeyValue:
			...
			model = None
		if model:
			test = multiTestClass(model=model, promptKeyValue=promptKeyValue, needTool=needTool, fileName=fileName, count=count)
			total = 0
			pprint.pprint(utils.prompt[promptKeyValue])
			for inputkeyValue, text in test.inputJson.items():
				test.logger.warning(f'Key: {inputkeyValue}')
				utils.prompt[promptKeyValue].append({'role': 'user', 'content': text})
				pprint.pprint({'role': 'user', 'content': text})
				for i in range(0, count):
					stepNumber = i + total
					test.logger.info('Processing step %s' % (stepNumber + 1))
					test.multiTester(inputkeyValue=inputkeyValue, text=text, row=test.data.empty_row + stepNumber)
					time.sleep(0.2)
					test.pbar.update(1)
				total += count
				utils.prompt[promptKeyValue].pop()
		else:
			print('Error: Invalid promptKeyValue')
	else:
		print('Usage: python3 multiTester.py [promptKeyValue] [needTool: 0, 1] [fileName] [count: int]')
